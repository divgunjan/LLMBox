...
export function AttentionSection({ inputText, selectedModel, attention = [], loading = false }: AttentionSectionProps) {
  ...
  useEffect(() => {
    ...
    const activeWeights = attention.length > 0 && attention[1]?.matrix ? attention[1].matrix : mockAttentionWeights;
    ...
    const cells = g
      .selectAll("rect")
      .data(activeWeights.flatMap((row: any[], i: number) => row.map((value: number, j: number) => ({ i, j, value }))))
      .enter()
      .append("rect")
      .attr("x", (d: any) => d.j * cellSize)
      .attr("y", (d: any) => d.i * cellSize)
      .attr("width", cellSize)
  ...
  }, [inputText, selectedModel, attention]);
